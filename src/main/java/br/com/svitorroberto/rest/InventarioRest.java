...
public class InventarioRest {
	...
	public Response cadastrarInventario(@PathParam("idItem")Long idItem,@PathParam("idPessoa")Long idPessoa) {
		...
		return result == "Item incluido com sucesso" ? Response.status(200).entity(result).build() : Response.status(417).entity(result).build();
	}
	...
	public Response removerInventario(@PathParam("idItem")Long idItem,@PathParam("idPessoa")Long idPessoa) {
		...
		return result == "Item removido com sucesso" ? Response.status(200).entity(result).build() : Response.status(417).entity(result).build();
	}
	
	@GET
	@Path("/{idPessoa1}/{itens1}/escambo/{idPessoa2}/{itens2}")
	@Consumes("application/json")
	public Response realizarEscambo(@PathParam("idPessoa1")Long idPessoa1,@PathParam("idPessoa2")Long idPessoa2, @PathParam("itens1")String itens1,@PathParam("itens2")String itens2) {
		ArrayList<String> itensPessoa1 = new ArrayList<>(Arrays.asList(itens1.split(",")));
		ArrayList<Item> item1 = new ArrayList<>();
		itensPessoa1.forEach(i -> {	item1.add(new Item(Long.valueOf(i)));});
		ArrayList<String> itensPessoa2 = new ArrayList<>(Arrays.asList(itens2.split(",")));
		ArrayList<Item> item2 = new ArrayList<>();;
		itensPessoa2.forEach(i -> {	item2.add(new Item(Long.valueOf(i)));});
		String result =  inventarioNegocio.realizarEscambo(new Escambo(new Usuario(idPessoa1), item1), new Escambo(new Usuario(idPessoa2), item2));
		return result == "Escambo realizado com sucesso" ? Response.status(200).entity(result).build() : Response.status(417).entity(result).build();
	}
